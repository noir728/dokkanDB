...
function renderZukanLayout() {
    ...
    if (!header || !contentDiv.contains(header)) {
        ...
        // Remove character button for team select mode
        const removeCharBtn = state.listMode === 'teamSelect' ?
            `<button class="remove-slot-btn" onclick="removeSlotAndReturn()">外す</button>` : '';

        // Removed Filter Button from header
        header.innerHTML = `
            <div class="search-container"><div class="search-wrapper">${removeCharBtn}<input type="text" id="zukan-search-input" class="search-bar" placeholder="名前、二つ名で検索..." value="${state.searchQuery}"><button id="search-clear-btn" class="search-clear-btn" onclick="clearSearch()">×</button></div></div>
    ...
    if (isSubSelectMode) {
        ...
        displayDB.forEach(char => {
            ...
            if (actualMode === 'icon') {
                ...
                item.style.position = 'relative';
                item.innerHTML = iconHtml;
                // 通常モードでも一応長押し詳細を入れておく（あるいは単純クリック）
                // リーダー選択時は長押しの必要性は低いが、統一しても良い
                // ここでは単純クリックのままにする（サブ選択以外は仕様変更なし）
